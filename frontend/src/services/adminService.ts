...
import type { Section, AdminMenuItem, TimeSlot, AdminProfile, VenueInfo } from "../types/admin";
...
export const adminService = {
  ...
  register: async (
    ...
    const { data } = await api.post("/register", { email, password, secretCode });
    return data;
  },
  ...
  // Cover Photo
  getCoverPhoto: async (): Promise<{ image: string }> => {
    const { data } = await api.get("/cover-photo");
    return data;
  },

  uploadCoverPhoto: async (file: File): Promise<{ image: string }> => {
    const form = new FormData();
    form.append("image", file);
